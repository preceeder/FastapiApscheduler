...
"""
-------------------------------------------------
    File Name:  newbook.py
    Description:  豆瓣图书信息爬虫
    Author:      Chenghu
    Date:       2022/2/26 12:54 下午 
-------------------------------------------------
    Change Activity:
-------------------------------------------------
"""
...
import httpx
import random
from apps.baseserver import SyncRedisBaseService, SyncMysqlBaseService
...
class NewBook:
    ...
    def save_db(self, data):
        # MysqlCnn.save_data(data, table='cw_book')
...
def start_book():
    try:
        ...
        # scheduler.modify_job('new_book', trigger=trigger, next_run_time=next_time)
        try:
            next_time = datetime.now() + timedelta(hours=random.randint(10, 24))
