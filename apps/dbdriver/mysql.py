...
from apps.settings.error_code import SQL_ERROR
...
class AioMysqlDb:
    ...
    async def connect_pool(self):
        ...
        if not self.host:
            return
        logger.info(f"连接异步数据库:{self.host}, -- {self.db}")
    ...
    async def select(self, sql: str, data: Union[dict, list] = None,
        ...
        """
        log_sql = sql.replace("\n", ' ')
        logger.info(f'select sql: {log_sql}; data: {data}; t_index: {t_index}')
        if t_index:
    ...
    async def execute(self, sql: str, data: Union[dict, list] = None, t_index: str = None) -> bool:
        ...
        """
        log_sql = sql.replace("\n", ' ')
        logger.info(f'execute: {log_sql}; data: {data}; t_index: {t_index}')
        if t_index:
            trans_cur = self.__transaction_cur[t_index]
            try:
                await trans_cur.execute(sql, data)
            except Exception as error:
                logger.error(f"sql error ：{log_sql} ； data: {data} ;\n error：{error}")
    ...
    async def commit(self, t_index: str) -> bool:
        ...
        try:
            ...
            if trans_conn is None:
                logger.error('t_index : trans fail because is t_index not find')
                return True
...
class pyMysqlDb(object):
    ...
    def __init__(self, conn_conf, pool_conf=None):
        ...
        logger.info(f"连接同步数据库:{self.host}, -- {self.db} -- {self.user} -- {self.pwd}")
    ...
    def select(self, sql: str, data: Union[dict, list] = None, many: bool = True,  t_index: str = None) -> Union[Dict, Tuple]:
        log_sql = sql.replace("\n", ' ')
        logger.info(f'select sql: {log_sql}; data: {data}; t_index: {t_index}')
        rows = None
    ...
    def execute(self, sql, data: dict = None, t_index: str = None) -> bool:
        ...
        relist = True
        log_sql = sql.replace("\n", ' ')
        logger.info(f'execute sql: {log_sql}; data: {data}; t_index: {t_index}')
        if t_index:
            trans_cur = self.__transaction_cur[t_index]
            ...
            try:
                ...
                relist = False
                logger.error("------------------SQL ERROR: {}".format(traceback.format_exc()))
            finally:
                cur.close()
                # self.conn.close()
        return relist
    ...
    def executemany(self, sql, data: list = None, t_index=None):
        ...
        :return:
        """
        logger.info("SQL: {}, data: {}".format(sql, data))
