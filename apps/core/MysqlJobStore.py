...
"""
-------------------------------------------------
    File Name:  MysqlJobStore.py
    Description:  写自己的mysql链接方式, 在这里你可以定义自己的数据表结构
    Author:      Chenghu
    Date:       2022/5/6 5:30 下午 
-------------------------------------------------
    Change Activity:
-------------------------------------------------
"""
...
class mysqlManager(object):
    ...
    def fetchmany(self, sql: str, data: dict = None) -> list:
        ...
        """
        if self.echo:
            logger.info(f"fetchmany: {sql} ; data: {data}")
    ...
    def fetchone(self, sql: str, data: dict = None) -> dict:
        ...
        """
        if self.echo:
            logger.info(f"fetchone: {sql} ; data: {data}")
        relist = {}
        cur = self._getConnectCur()
        try:
            cur.execute(sql, data)
    ...
    def execute(self, sql: str, data: dict = None) -> bool:
        ...
        relist = 0
        if self.echo:
            logger.info(f"execute: {sql} ; data: {data}")
...
class MysqlJobStore(BaseJobStore):
    ...
    def add_job(self, job):
        try:
            ...
            logger.error(f'add job error : {type(e)}')
