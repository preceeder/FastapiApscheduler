...
"""
-------------------------------------------------
    File Name:  dbinterface.py
    Description:  
    Author:      Chenghu
    Date:       2022/2/12 10:26 上午 
-------------------------------------------------
    Change Activity:
-------------------------------------------------
"""
from typing import Union, Tuple, Dict
from apps.dbdriver import mysql, redis, sync_redis, async_mysql
from apps.settings.product.db_config import TABLE_DB_MAP
...
class DBInterface(object):
    ...
    #
    # async def fetchmany(self, table_name, sql, **kwargs):
    #     raise NotImplementedError
    #
    # async def execute(self, table_name, sql, **kwargs):
    #     raise NotImplementedError
    #
    # async def fetchmany_with_total(self, table_name, sql,**kwargs):
    #     raise NotImplementedError
...
class MySQLDBInterface(DBInterface):
    ...
    async def execute(self,  sql: str, table_name: str = None, data: Union[dict, list] = None, t_index: str = None) -> bool:
        """
        :param sql: 待执行的SQL语句
    ...
    async def executemany(self, sql: str, table_name: str = None, data: Union[dict, list] = None, t_index: str = None)-> bool:
        db = self._get_db(table_name)
...
class SyncMysqlDBInterface(DBInterface):
    ...
    def executemany(self, sql: str, table_name: str = None, data: Union[dict, list] = None, t_index: str = None)-> bool:
        db = self._get_db(table_name)
